...
state CombatSword in W3PlayerWitcher extends Combat 
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		// Dragnilar -  AutoApplyOils 1 of 2
		var AutoApplyOilsInstance : AutoApplyOils;
		// Dragnilar - End AutoApplyOils 1 of 2
		...
		// Dragnilar -  AutoApplyOils 2 of 2
		AutoApplyOilsInstance = new AutoApplyOils in this;
		AutoApplyOilsInstance.Initialise();
		delete AutoApplyOilsInstance;
		// Dragnilar - End AutoApplyOils 2 of 2	
	}
	...
	{	
		...
		if ( parent.GetBehaviorVariable( 'isPerformingSpecialAttack' ) == 0.f )
		{
			if ( enableAttack )
			{
				...
				//Dragnilar - Quen for Rend
				if (GetWitcherPlayer().CanUseSkill(S_Sword_s02) && GetWitcherPlayer().GetSkillLevel(S_Sword_s02) > 1)
				{
					GetWitcherPlayer().RestoreQuen( 1000000.f, 10.f, true );
					FactsAdd("quen_for_rend", 1, 4 );
				}
			}
		}
		else
	...
	{
		var quen : W3QuenEntity; //Dragnilar - Quen for Rend

		theGame.GetGameCamera().StopAnimation( 'camera_shake_loop_lvl1_1' );
		parent.UnblockAction( EIAB_Crossbow, 'SpecialAttack' );
		parent.RemoveTimer('SpecialAttackHeavySustainCost');
		//Dragnilar - Quen for Rend
		if (FactsQuerySum("quen_for_rend") > 0)
		{
			...
			FactsRemove("quen_for_rend");
		}
		//Dragnilar - Yrden sword effect
	...
	//Dragnilar - Taken from the Magic Spells mod, credit goes to menschfeind13
	public function YrdenSword()
	{
		var shootposition, weaponTipPosition 	: Vector;
		var rot 			: EulerAngles; 
		var entity 	: CGameplayEntity;
		var effect_entity, targetent, sword_for_effect, effect : CEntity;
		...
		length = AbsF(VecDistance(weaponTipPosition, sword_for_effect.GetWorldPosition()));

		geralt = GetWitcherPlayer();
		shootposition = geralt.GetWorldPosition() - geralt.GetHeadingVector()*2;
		rot = geralt.GetWorldRotation();
		rot.Roll += 90;
		shootposition.Z-=2;
		effect_entity = (CEntity)theGame.CreateEntity( (CEntityTemplate) LoadResource( "dlc\DragnilarEdit\swordeffects\igni_heavy.w2ent",true ),
		shootposition, rot);
		...
		shootposition = geralt.GetWorldPosition() + geralt.GetHeadingVector()*2;
		aard_wave = (W3TraceGroundProjectile)theGame.CreateEntity( (CEntityTemplate) LoadResource( "dlc\DragnilarEdit\swordeffects\yrden_proj.w2ent",true ), 
		shootposition, geralt.GetWorldRotation());
		aard_wave.Init(NULL);
		aard_wave2 = (W3TraceGroundProjectile)theGame.CreateEntity( (CEntityTemplate) LoadResource( "dlc\DragnilarEdit\swordeffects\yrden_proj2.w2ent",true ), 
		shootposition, geralt.GetWorldRotation());
		aard_wave2.Init(NULL);

		targetent = (CEntity)theGame.CreateEntity( (CEntityTemplate) LoadResource( "aard",true ), 
			geralt.GetWorldPosition() + geralt.GetHeadingVector()*5, geralt.GetWorldRotation());
		...
		for( n=0.2; n<=length; n+=0.1 )
		{	
			effect = theGame.CreateEntity( (CEntityTemplate) LoadResource( "dlc\adtomes\DragnilarEdit\swordeffects\effects_for_sword.w2ent",true ), thePlayer.GetWorldPosition(), thePlayer.GetWorldRotation() );
